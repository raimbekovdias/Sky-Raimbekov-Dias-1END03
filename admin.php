...
<body style="background: url(dlyatexta2.jpg);">
			...
			<div class="col-md-3" style="background-color:#FF4A75; min-height:200px;font-size:15px; width:439px">
				...
				<form style="text-align:center; margin-top:10px;"  name="MyForms" autocomplete="on" method='post' action='register/testreg.php'>
					...
					<p>
						...
						<input   name='password' placeholder="Пароль" required style ="margin-right:3px;width:216px" >
					</p>
	...
	<?php
		...
		?>
<table class="table table-inverse" style="color:white">

		<tr><td>name</td><td>username</td><td>email</td><td>comment</td><td>date</td><td>delete</td></tr>
		<?php
		while($row=mysql_fetch_assoc($result)){
			$sqll="select * from user where id=".$row['user_id'];
...
<table class="table table-inverse" style="color:white">
		...
		<tr><td>name</td><td>username</td><td>email</td><td>password</td><td>delete</td></tr>
			<?php
			...
			<tr>
				...
				<td><a href="deleteuser.php?id=<?php echo $r['id']; ?>">delete</a></td>
			</tr>
			<?php
		} 
	?>
